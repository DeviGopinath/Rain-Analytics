...
public class RedisRepositoryImpl implements RedisRepository {
    ...
    public List<String> findAll(){
    	try { 
    		Map<String, String> avalue = redisTemplate.<String, String>opsForHash().entries(MDO_REPO);
    		return avalue.values().stream().collect(Collectors.toList()); 
    	} catch (Exception e) {
    		LOGGER.error(String.format("Encountered an error while finding all values for Default Key %s" , e.getMessage()));
    	}
    	return null;
    }
    ...
    public List<String> findAllForKey(String key) { 
    	try { 
    		...
    		LOGGER.error(String.format("Encountered an error while finding all values for Key :: %s" ,   e.getMessage()));
    	}
    	return null;
    }
		...
		try {
			...
			queries.forEach(query -> {
				...
				try {
					...
					LOGGER.error(String.format("Encountered an Exception while converting Request Query Map: %s" , ex.getMessage()));
				}
			});
			
			if (request.getFilters() != null) {
				applyFilters(avalue, request, requestQueryMap);
			}
			JsonNode aggrNode = new ObjectMapper().convertValue(avalue, JsonNode.class);
			return aggrNode;
		} catch (Exception e) {
			LOGGER.error(String.format("Encountered an error while finding all values for Key : %s :: %s" ,key , e.getMessage()));
		}
		return null;
	}
    
	public Map<String, String> applyFilters(Map<String, String> avalue, AggregateRequestDto request, Map<String, Map<String, String>> requestQueryMap) {
		...
		Iterator<Entry<String, Map<String, String>>> rQMItr = requestQueryMap.entrySet().iterator(); 
		 Map<String, String> mapOfAllRequestQueryMaps = new HashMap<>(); 
		while (itr.hasNext()) {
